# LongXi Development Guidelines

This file provides guidance to CODEX Code (CODEX.ai/code) when working with code in this repository.

## 1. Purpose

This file defines how CODEX Code operates within this repository. It is an internal engineering instruction manual that governs:

- How CODEX reasons about code changes
- How CODEX approaches implementation tasks
- How CODEX respects architecture and design constraints
- How CODEX interacts with specifications and design documents
- How CODEX handles uncertainty and ambiguity
- How CODEX restricts itself to safe change patterns
- How CODEX formats responses and code changes

This is not a project design document, product specification, or roadmap. It is a behavioral constraint system for CODEX Code.
...
CODEX Code must adhere to these working principles:
...
When multiple sources conflict, CODEX must follow this priority order:
...
- If documentation and code conflict, CODEX must identify the conflict explicitly
- CODEX must not invent certainty when the repository is ambiguous
- CODEX must distinguish between: confirmed facts, reasonable assumptions, and recommendations
- When making assumptions, CODEX must state them clearly as assumptions

## 6. Spec-First Workflow

When the repository uses a specification-driven development process (e.g., Spec-Kit), CODEX must:
...
CODEX must make code changes according to these rules:
...
CODEX must respect architectural boundaries and constraints:
...
When investigating or fixing bugs, CODEX must:
...
When faced with uncertainty, CODEX must:
...
CODEX must maintain documentation integrity:
...
CODEX must verify changes to the extent the environment allows:
...
CODEX must communicate in a professional, engineering-focused manner:
...
CODEX must interact with the repository in a disciplined way:
...
CODEX Code must adhere to these naming and style rules:
...
CODEX Code must adhere to these dependency management rules:
...
- CODEX MUST prefer updating the centralized dependency authority rather than scattering dependency changes across the repo
...
CODEX Code must adhere to these library interface rules:
...
CODEX Code must adhere to these external dependency rules:
