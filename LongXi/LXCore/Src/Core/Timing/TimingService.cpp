...
void TimingService::SetFrameRateLimit(double targetFramesPerSecond)
{
    ...
    {
        LX_CORE_WARN(
            "[Timing] Invalid frame rate limit requested ({:.3f}). Keeping {:.3f} FPS", targetFramesPerSecond, m_TargetFramesPerSecond);
        return;
    }
...
void TimingService::SetMaxDeltaTime(double maxDeltaSeconds)
{
    ...
    {
        LX_CORE_WARN("[Timing] Invalid max delta requested ({:.6f}). Keeping {:.6f} seconds", maxDeltaSeconds, m_MaxDeltaSeconds);
        return;
    }
...
void TimingService::BeginFrame()
{
    ...
    {
        LX_CORE_WARN("[Timing] Large frame gap detected. Unscaled {:.3f} ms clamped to {:.3f} ms",
