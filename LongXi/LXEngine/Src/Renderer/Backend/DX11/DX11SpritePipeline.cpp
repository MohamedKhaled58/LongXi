                                    ...
                                    const char* pixelShaderSource)
{
    ...
    HRESULT hr =
        D3DCompile(vertexShaderSource, strlen(vertexShaderSource), nullptr, nullptr, nullptr, "VS", "vs_4_0", 0, 0, &vsBlob, &errorBlob);
    if (FAILED(hr))
    {
...
{
    return m_Initialized && renderer.IsInitialized() && renderer.GetNativeContextHandle() != nullptr && m_VertexBufferHandle.IsValid() &&
           m_IndexBufferHandle.IsValid() && m_ConstantBufferHandle.IsValid();
}
