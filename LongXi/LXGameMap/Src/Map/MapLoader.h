...
private:
    ...
    static bool ParseAni(const std::string& aniPath, CVirtualFileSystem& vfs, std::unordered_map<uint16_t, std::vector<std::string>>& outFramesByPuzzle, std::vector<std::string>& outWarnings);

    static bool ResolveMapPath(
    ...
    static bool TryResolveTexturePath(const std::string& framePath, const MapDescriptor& descriptor, CVirtualFileSystem& vfs, std::string& outResolvedPath);
    static void BuildTexturePathCandidates(const std::string& framePath, const MapDescriptor& descriptor, std::vector<std::string>& outCandidates);
    static bool WorldToTile(const MapDescriptor& descriptor, int32_t worldX, int32_t worldY, int32_t& outTileX, int32_t& outTileY);
    static bool ParsePassageBlock(const std::vector<uint8_t>& bytes, size_t& ioCursor, std::vector<MapPassageRecord>& outPassages, std::vector<std::string>& outWarnings);
    ...
    static bool ParseSceneLayerBlock(const std::vector<uint8_t>& bytes,
                                     ...
                                     bool& outAborted,
                                     std::vector<std::string>& outWarnings);
    static bool ReadFixedBlock(const std::vector<uint8_t>& bytes, size_t& ioCursor, size_t blockBytes, const char* blockName, std::vector<uint8_t>& outBlock, std::vector<std::string>& outWarnings);
