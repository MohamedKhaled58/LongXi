...
void CameraPanel::Render(const CameraStateViewModel& cameraState, Camera& camera)
{
    ...
    if (ImGui::DragFloat3("##Position", position, 0.1f))
    {
        ...
        LX_INFO("[CameraPanel] Position changed: ({}, {}, {})", position[0], position[1], position[2]);
    }
    ...
    if (ImGui::DragFloat3("##Rotation", rotation, 1.0f, -180.0f, 180.0f))
    {
        ...
        LX_INFO("[CameraPanel] Rotation changed: ({}, {}, {})", rotation[0], rotation[1], rotation[2]);
    }
    ...
    {
        camera.SetFOV(fov);
        LX_INFO("[CameraPanel] FOV changed: %.1f°", fov);
    }
    ...
    if (ImGui::DragFloat("Near Plane", &nearPlane, 0.01f, 0.001f, 100.0f, "%.3f"))
    {
        ...
        LX_INFO("[CameraPanel] Near plane changed: %.3f", nearPlane);
    }
    ...
    if (ImGui::DragFloat("Far Plane", &farPlane, 10.0f, 1.0f, 100000.0f, "%.1f"))
    {
        ...
        LX_INFO("[CameraPanel] Far plane changed: %.1f", farPlane);
    }
